#
# @lc app=leetcode id=966 lang=python
#
...
class Solution(object):
    def spellchecker(self, wordlist, queries):
        ...
        for word in wordlist:
            word_caps.setdefault(word.lower(), word)
            word_devowel.setdefault(devowel(word.lower()), word)
        ...
        for query in queries:
            if query in word_perfect:
                ...
                continue
            query = query.lower()
            if query in word_caps:
                res.append(word_caps[query])
                continue
            query = devowel(query)
            if query in word_devowel:
                res.append(word_devowel[query])
                continue
            res.append("")
        return res
