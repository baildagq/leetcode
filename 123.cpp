...
public:
    /*
    // method-1: 暴力 超时，O(n^2)复杂度不可接受
    int maxProfit(vector<int> &prices)
    {
        int size = prices.size();
        vector<vector<int>> profit(size, vector<int>(size, 0)); // profit[i][j] means buy at day[i] && sell at day[j]'s profit
        vector<int> maxProfitFront(size, INT_MIN);
        vector<int> maxProfitBack(size, INT_MIN);
        ...
        {
            maxProfitFront[i] = max(maxProfitFront[i], maxProfitFront[i - 1]);
            for (int j = 0; j <= i; j++)
            {
                maxProfitFront[i] = max(maxProfitFront[i], profit[j][i]);
            }
        }
        maxProfitBack[size - 1] = 0;
        for (int i = size - 2; i >= 0; i--)
        {
            maxProfitBack[i] = max(maxProfitBack[i], maxProfitBack[i + 1]);
            for (int j = i; j < size; j++)
            {
                maxProfitBack[i] = max(maxProfitBack[i], profit[i][j]);
            }
        }
        int res = 0;
        res = max(res, maxProfitFront[size - 1]);
        for (int i = 1; i < size - 1; i++)
        {
            res = max(res, maxProfitFront[i] + maxProfitBack[i + 1]);
        }
        res = max(res, maxProfitFront[0]);
        return res;
    }
    */
    ...
    /*
    // method-2: dfs + 记忆化搜索 也相当于暴力, 注意的是因为默认的unordered_map不支持以vector作为键，需要自己实现hash和判等函数，所以没有继续写下去
    // 如果python的话，就会相对好些很多
    int dfs(vector<int>& prices, int status, int num, int index)
    {
        if (index == prices.size() || num == 2)
        {
            return 0;
        }
        int profit = dfs(prices, status, num, index + 1);
        if (status == 1)
        {
            // 持有，卖出
            profit = max(profit, dfs(prices, (status + 1) % 2, num + 1, index + 1) + prices[index]);
        }
        else
        {
            // 非持有，买入
            profit = max(profit, dfs(prices, (status + 1) % 2, num, index + 1) - prices[index]);
        }
        return profit;
    }
    int maxProfit(vector<int> &prices)
