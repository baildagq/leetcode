...
要求：空间复杂度为O(1)
方法：
1. 暴力：每次旋转一个长度，则空间复杂度为1，旋转k次，时间复杂度为O(nk)
2. 环状替换：会发现有两种情况
    1. nums.size() % k == 0: 会出现多个环
    2. nums.size() % k != 0: 会出现一个替换链条

#### 195. 第十行
bash编程题，但是自己确实啥也不会，看答案都要查好久相关指令才能看懂
1. grep 
2. wc -l
3. sed -n '10p'
4. awk '{if(NR==10){print $0}}'


## 2020.10.07
#### 192.统计词频
使用文本处理工具，涉及到cat,grep,xargs,awk,sort,uniq等工具的组合使用
...
#### 164. 最大间距
进阶要求：O(n)的时间与空间复杂度
找到一个无序数组排序后最大的相邻元素间的间距
正常思路为排序+遍历，但是常见的比较排序的时间复杂度都为O(nlogn)，想要O(n)的时间复杂度，需要将排序部分改为O(n)，比如基数排序
非正常思路看了题解，发现了一个基于桶的做法，但是确实没有理解qaq

(note: 之后不会在本地存一份代码了，因为，想要看题还是要上leetcode，可以在提交记录中找到，本地维护感觉不是很有意义，之后要更关注每道题做完之后的收获和思考了)

#### 165. 比较版本号
python用于处理字符串真的很方便，主要是可以直接split了存成一个list了
c++实现感觉就会比较麻烦了，首先这个没有split就要手动去判断生成数了

#### 169. 多数元素
因为之前尝试写过O(1)的求众数，学习了摩尔投票法，所以这个题也比较轻松的写出了O(1)的做法
不过也看到了另一种比较好的做法，就是利用排序后，数组中间位置的元素一定是超过'下取整(n/2)'的数字的性质

#### 179. 最大数
自己写了好半天的直接比较两个字符串的在本题目中的大小，最后也没写出来
后来看题解，可以直接尝试拼接两个字符串，在题目要求意义下进行比较，这样一想之前自己真的太蠢了
学习到了如果自定义一个sort比较函数，记忆一下

#### 785. 判断二分图
dfs, 写的很慢，而且最后思路是一样的，但是写出来的代码和题解中相比就感觉啰嗦了好多
...
method-2: **归并排序**，排序过程就是消除队列中的逆序对的过程，在归并排序的过程中统计逆序对，时间复杂度O(nlogn)

#### [674. 最长连续递增序列](https://leetcode-cn.com/problems/longest-continuous-increasing-subsequence/)

感觉写的还是有些丑陋，因为现在写法特判了第一个和最后一段

#### [673. 最长递增子序列的个数](https://leetcode-cn.com/problems/number-of-longest-increasing-subsequence/)

method-1: 动态规划，求解以nums[i]为结尾的最大上升子序列的长度和数量
len[i] : 表示最大上升子序列的长度
num[i] : 表示最大上身子序列的数量
初值为 : len[i] = 1 num[i] = 1
